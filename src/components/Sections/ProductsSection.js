...
export default function ProductsSection({ reverst, title, advantages, children, image }){
  return(
    <Section>
      <GridBox className='lg:grid-cols-2 items-center m-8 justify-items-center gap-8'>
        <Container>
            <H3 className='text-4xl font-extrabold'>{title}</H3>
            <P className='my-6'>{children}</P>
            <FlexBox className='gap-3 sm:flex-row flex-col mb-6 sm:mb-0'>
        ...
        <Img url={image} className={`${reverst && '-order-1'} hidden lg:block self-stretch`}/>
      </GridBox>
    </Section>
  );
}